...
from .backdoor_dataset import CIFAR10Mem, CIFAR10Pair, BadEncoderTestBackdoor, BadEncoderDataset, ReferenceImg, BadEncoderDataset_shiwei
import numpy as np
...
def get_pretraining_cifar10_partial(data_dir):
    training_data_num = 50000
    testing_data_num = 10000
    np.random.seed(100)
    training_data_sampling_indices = np.random.choice(training_data_num,int(training_data_num*0.05), replace=False)
    train_data = CIFAR10Pair(numpy_file=data_dir + "train.npz", class_type= classes, transform=train_transform, indices=training_data_sampling_indices)
    memory_data = CIFAR10Mem(numpy_file=data_dir + "train.npz", class_type= classes, transform=test_transform_cifar10)
    test_data  = CIFAR10Mem(numpy_file=data_dir + "test.npz", class_type= classes,transform=test_transform_cifar10)
...
def get_shadow_cifar10(args):
    ...
    training_data_sampling_indices = np.random.choice(training_data_num,int(training_data_num*args.shadow_fraction), replace=False)
    print(training_data_sampling_indices)
...
def get_shadow_cifar10_224_shiweiimagenet(args):
    training_data_num = 50000
    testing_data_num = 10000
    np.random.seed(100)
...
def get_downstream_cifar10(args):
    ...
    if args.shadow_dataset in ['imagenet_nju']:
        trigger_size=64
    else:
        trigger_size=None
    ...
    test_data_backdoor = BadEncoderTestBackdoor(numpy_file=args.data_dir+testing_file_name, trigger_file=args.trigger_file, reference_label= args.reference_label,  transform=test_transform, trigger_size=trigger_size)
